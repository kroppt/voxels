...
func TestCameraVoxelCoords(t *testing.T) {
	testCases := []struct {
		desc   string
		cPos   glm.Vec3
		expect oldworld.VoxelPos
	}{
		{
			desc:   "pos pos works",
			cPos:   glm.Vec3{0.5, 0.5, 0.5},
			expect: oldworld.VoxelPos{0, 0, 0},
		},
		{
			desc:   "pos neg works",
			cPos:   glm.Vec3{0.5, 0.5, -0.5},
			expect: oldworld.VoxelPos{0, 0, -1},
		},
		{
			desc:   "neg pos works",
			cPos:   glm.Vec3{-0.5, 0.5, 0.5},
			expect: oldworld.VoxelPos{-1, 0, 0},
		},
		{
			desc:   "neg neg works",
			cPos:   glm.Vec3{-0.5, 0.5, -0.5},
			expect: oldworld.VoxelPos{-1, 0, -1},
		},
		{
			desc:   "far neg neg works",
			cPos:   glm.Vec3{-1.5, -2.5, -1.5},
			expect: oldworld.VoxelPos{-2, -3, -2},
		},
		{
			desc:   "far pos pos works",
			cPos:   glm.Vec3{1.5, 2.5, 1.5},
			expect: oldworld.VoxelPos{1, 2, 1},
		},
	}
	for _, tC := range testCases {
