...
func TestGetChunkIndex(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			chunkSize: 3,
			expect:    world.ChunkPos{-2, 0, 0},
		},
		{
			desc:      "negative y no change",
			pos:       world.VoxelPos{1, -1, 1},
			chunkSize: 3,
			expect:    world.ChunkPos{0, -1, 0},
		},
		{
...
func TestGetChunkBounds(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			desc:       "3x3 around 0,0",
			renderDist: 1,
			currChunk:  world.ChunkPos{0, 0, 0},
			expectRng: world.ChunkRange{
				Min: world.ChunkPos{X: -1, Y: -1, Z: -1},
		...
		},
		{
			desc:       "5x5 around -1,-1",
			renderDist: 2,
			currChunk:  world.ChunkPos{-1, 0, -1},
			expectRng: world.ChunkRange{
				Min: world.ChunkPos{X: -3, Y: -2, Z: -3},
				Max: world.ChunkPos{X: 1, Y: 2, Z: 1},
			},
		},
		{
			desc:       "1x1 around 3,5",
			renderDist: 0,
			currChunk:  world.ChunkPos{3, 0, 5},
			expectRng: world.ChunkRange{
				Min: world.ChunkPos{X: 3, Y: 0, Z: 5},
				Max: world.ChunkPos{X: 3, Y: 0, Z: 5},
			},
		},
		{
			desc:       "3x3 around -1,2",
			renderDist: 1,
			currChunk:  world.ChunkPos{-1, 0, 2},
			expectRng: world.ChunkRange{
				Min: world.ChunkPos{X: -2, Y: -1, Z: 1},
				Max: world.ChunkPos{X: 0, Y: 1, Z: 3},
			},
		},
	}
	for _, tC := range testCases {
